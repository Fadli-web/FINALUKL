...
export default function CashierDashboardPage() {
  ...
  // 🛠️ PERBAIKAN: Menambahkan variasi key standar backend untuk data detail pesanan
  const getOrderItems = (order: any): any[] =>
    order.orderDetails ||
    order.order_details ||
    order.OrderDetails ||
  ...
  // 🛠️ PERBAIKAN: Logika kalkulasi yang lebih kuat mencocokkan struktur JSON Postman
  const totalRevenue = completedOrders.reduce((total, order) => {
    // 1. Jika backend sudah memberikan field total harga di tabel transaksi (langsung pakai)
    if (order.total_price) return total + Number(order.total_price);
    if (order.totalPrice) return total + Number(order.totalPrice);
    if (order.total) return total + Number(order.total);

    // 2. Jika tidak ada field total, kita hitung akumulasi dari item detailnya
    const items = getOrderItems(order);
    const orderTotal = items.reduce((sub: number, item: any) => {
      // Ambil ID Menu
      const menuId = item.menuId || item.menu_id || item.MenuId;
      // Cocokkan dengan data menu
      const matched = item.menu || item.Menu || menus.find((m) => m.id === menuId);
      
      // Ambil Harga dan Qty
      const price = item.price || item.harga || (matched ? matched.price : 0);
      const qty = item.quantity || item.qty || 1;
      
      return sub + (Number(price) * Number(qty));
    }, 0);
