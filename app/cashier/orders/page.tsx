...
export default function CashierOrdersPage() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const sorted = (orderRes.data || orderRes || []).sort(
        (a: any, b: any) => b.id - a.id
      );
      setOrders(sorted);
  ...
  return (
    <div
      style={{ background: "#0d0a08", minHeight: "100vh" }}
      className="text-stone-100 pb-16 font-sans"
    >
      {/* Grain overlay */}
      <div
        className="fixed inset-0 pointer-events-none opacity-[0.03] z-0"
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-4xl sm:text-5xl font-bold leading-none tracking-tight" style={{ color: "#f5e6d0" }}>
              Live<br /><span style={{ color: "#c8923a" }}>Kitchen</span>
            </h1>
          </div>
          <div className="flex items-center gap-3">
              ...
              >
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <h3
                        className="font-bold text-base leading-none mb-2"
                        style={{ color: "#f5e6d0", fontFamily: "'Courier New', monospace" }}
                      >
                        #ORDER-{order.id}
                      </h3>
                      <div 
                        className="text-[10px] tracking-widest uppercase flex flex-col gap-1" 
                        style={{ color: "#8a7560", fontFamily: "'Courier New', monospace" }}
                      >
                        {/* Info Pemesan */}
                        <p>
                          {order.user?.username ? (
                            <>
                              <span style={{ color: "#c8b89a" }}>{order.user.username}</span> (ID: {order.userId})
                            </>
                          ) : order.userId ? (
                            `Member ID: ${order.userId}`
                          ) : (
                            "Pelanggan Umum"
                          )}
                        </p>
                ...
                <div className="px-5 py-4 flex-1 space-y-3">
                  {items.length === 0 && (
                    <p className="text-xs italic text-center text-stone-500">
                      — Data item tidak tersedia —
                    </p>
                  )}
                  {items.map((item: any, idx: number) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="flex items-center justify-between gap-3 bg-black/20 p-2.5 rounded-lg border border-white/5"
                      >
                        <span
                          className="text-sm font-medium line-clamp-2"
                          style={{ color: "#c8b89a", lineHeight: "1.3" }}
                        >
                          {matchMenu?.name || `Produk #${item.menuId || item.menu_id}`}
                        </span>
                        <div
