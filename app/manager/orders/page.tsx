...
interface OrderItem {
  ...
  menu?: { id: number; name: string; price: string | number };
}
...
interface Order {
  ...
  createdAt: string;
  items?: OrderItem[];
  orderItems?: OrderItem[]; // Mengakomodasi jika nama properti API adalah orderItems
...
function formatRupiah(amount: string | number | undefined) {
...
// Resolvers
function resolveItems(order: any): OrderItem[] {
  return order.items || order.orderItems || order.order_items || order.detail_order || [];
}
...
function ManagerOrderCard({
  ...
  return (
    ...
    >
      ...
      {/* 3. UPDATE: Area Expandable (Rincian Pesanan & Pembayaran) */}
      <div className={`overflow-hidden transition-all duration-300 ease-in-out ${expanded ? "max-h-[1000px] opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="border-t border-white/[0.06] bg-black/20 p-5 space-y-4">
          ...
          {/* Info Pembayaran */}
          <div className="flex flex-wrap gap-4 bg-white/[0.02] p-4 rounded-xl border border-white/[0.04]">
            <div className="flex-1 min-w-[120px]">
              <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1 font-bold">Total Harga</p>
              <p className="text-lg font-black text-amber-400">{formatRupiah(order.totalPrice)}</p>
            </div>
            <div className="flex-1 min-w-[120px]">
              <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1 font-bold">Metode Pembayaran</p>
              <p className="text-sm font-bold text-zinc-200 uppercase">
                {order.paymentMethod ? order.paymentMethod : "Belum Dipilih"}
              </p>
            </div>
            <div className="flex-1 min-w-[120px]">
              <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1 font-bold">Status Bayar</p>
              <span className={`inline-flex px-2.5 py-1 rounded-md text-[10px] font-black tracking-widest ${order.isPaid ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-rose-500/20 text-rose-400 border border-rose-500/30'}`}>
          ...
          <div className="space-y-3">
            ...
            {items.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {items.map((item, i) => (
                  <div key={i} className="flex items-center justify-between gap-3 bg-white/[0.02] p-3 rounded-xl border border-white/[0.04]">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-white/[0.06] border border-white/[0.08] flex items-center justify-center text-white font-black text-xs flex-shrink-0">
                        {item.quantity}x
                      </div>
                      <div>
                        ...
                        {item.unitPrice && (
                          <p className="text-xs text-zinc-500">{formatRupiah(item.unitPrice)} / item</p>
                        )}
                      </div>
                    </div>
                    {item.subtotal && (
                      <p className="text-sm font-bold text-zinc-300">{formatRupiah(item.subtotal)}</p>
                    )}
                  </div>
                ))}
              </div>
...
export default function ManagerOrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      // Membuat map untuk mempercepat pencarian nama menu
      const map: Record<number, Menu> = {};
      ((menuRes.data || menuRes) as Menu[]).forEach((m) => { map[m.id] = m; });
      setMenuMap(map);
    } catch {
