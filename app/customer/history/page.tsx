...
interface Order {
  ...
  createdAt?: string;
  items: OrderItem[];
  // Tambahan relasi user untuk mengambil username (sesuaikan jika nama field dari backend berbeda)
  user?: {
    id: number;
    username: string;
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ID dan Info Pemesan (Ditambahkan di sini) */}
        <div className="flex-shrink-0">
          <p className="text-white font-black tabular-nums text-base">
            #CAFE‑{String(order.id).padStart(4, "0")}
          </p>
          <div className="flex flex-col mt-0.5 space-y-0.5">
            <p className="text-zinc-400 text-xs font-medium flex items-center gap-1.5">
              ...
              {/* Cek apakah ada data username dari backend, jika tidak, tampilkan ID-nya saja */}
              {order.user?.username ? (
                <span>
                  {order.user.username} <span className="text-zinc-600">(ID: {order.userId})</span>
                </span>
              ) : (
                <span>ID Pemesan: {order.userId}</span>
              )}
            </p>
            <p className="text-zinc-600 text-[10px]">{items.length} variasi item (Klik untuk melihat detail)</p>
          </div>
        </div>
      ...
      {/* Expandable detail (Rincian Pesanan) */}
      <div
        className={`overflow-hidden transition-all duration-300 ${expanded ? "max-h-screen" : "max-h-0"}`}
      >
        <div className="border-t border-white/[0.06] px-5 pt-4 pb-5 space-y-4">
...
export default function CustomerHistoryPage() {
  ...
  return (
    <div className="space-y-6 p-1 max-w-2xl mx-auto">
      ...
      {!loading && filtered.length > 0 && (
        <p className="text-center text-zinc-600 text-xs mt-6">{filtered.length} transaksi ditampilkan</p>
      )}
    </div>
  );
}